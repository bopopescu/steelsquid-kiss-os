...
Also see radiorover.html (called from utils.html)
...
def enable(argument=None):
    ...
    steelsquid_kiss_global.clear_modules_settings("kiss_radiorover")
...
class SYSTEM(object):
    ...
    def on_start():
        ...
        if SETTINGS.is_master:
            steelsquid_utils.shout("Steelsquid Radio Rover enabled")
            ...
            steelsquid_utils.shout("Steelsquid Radio Rover controller enabled")
        ...
        if SETTINGS.is_master:
            try:
                ...
                if drive_delta.total_seconds()>2:
                    GLOBAL.drive(0, 0)
                urllib.urlretrieve("http://localhost:8080/?action=snapshot", "/opt/steelsquid/web/tmpfs/radiorover.jpg")
                the_pic=None
                with open("/opt/steelsquid/web/tmpfs/radiorover.jpg", mode='rb') as f:
...
class WEB(object):
    ...
    @staticmethod
    def radiorover_settings(session_id, parameters):
        '''
    ...
    @staticmethod
    def radiorover_status(session_id, parameters):
        '''
        Get status
        '''
        return []
    ...
    @staticmethod
    def radiorover_camera(session_id, parameters):
        '''
    ...
    @staticmethod
    def radiorover_drive(session_id, parameters):
        '''
        Tilt camera up and down
        '''
    ...
    @staticmethod
    def radiorover_lamp(session_id, parameters):
        '''
...
class RADIO(object):
    ...
    def on_receive(data):
        ...
        '''
        with open("/opt/steelsquid/web/tmpfs/radiorover_t.jpg", mode='wb') as f:
            f.write(data)
        if os.path.getsize('/opt/steelsquid/web/tmpfs/radiorover_t.jpg')>1:
            os.rename("/opt/steelsquid/web/tmpfs/radiorover_t.jpg", "/opt/steelsquid/web/tmpfs/radiorover.jpg")
