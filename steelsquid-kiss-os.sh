...
function help_build()
{
    ...
    echo "unxz raspbian-ua-netinst-v1.0.5.img.xz"
    echo 
    echb "2.  Copy to sdcard"
    echo "dd bs=4M if=raspbian-ua-netinst-v1.0.5.img of=/dev/sdb"
    echo 
    ...
    echo 
    echb "10.  Resize to 1.8G with gparted"
    echo 
...
if [ "$in_parameter_1" == "read" ]; then
	...
	dd if=$in_parameter_2 of=./$project_name.img bs=4M count=461
	exit-check
